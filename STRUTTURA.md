...
- **evento in vendita** = l'**ultimo caricato** (data più alta; il giorno dell'evento si vendono già i biglietti del successivo): `MB21Lista.eventoAttivo(eventi, quando)`; per un giorno passato conta solo quello che era già caricato (`creato_il`). `MB21Lista.momento(t)` legge le date del database anche su Safari (massimo 3 decimali)
