...
- **Registrazione** (cantiere 19, decisioni di Ignazio 16/09: approva **solo l'Admin**, codice Amway obbligatorio): indirizzo con `?registrati` (e `&da=<id utente>`) **senza essere entrati** → modulo «Registrati a MB21» (nome e cognome, email, telefono, codice Amway) → `richiedi_accesso` → «Richiesta inviata ✅» o messaggio (già registrato, già richiesta, troppe, dati). «Sono già registrato: entra» toglie `?registrati` e mostra l'accesso. Da entrati il link apre l'app normale. Cambia la decisione del brief Fase 1 («nessuna registrazione libera»): la registrazione c'è ma serve l'approvazione
