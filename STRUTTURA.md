...
- **Lista Nomi** (Fase 2, copia della tab di Glide → `docs/MB21_v3_Lista_come_e.md`):
  ...
  - **Segni vitali** (cantiere 18 lavoro 2, 16/09): riquadro **Compagno/a** (nome e telefono, «Modifica» → foglio; lo salva chi può modificare il contatto) · riquadro **BBS** e riquadro **WES**: un blocco per evento (dal più recente) con «N biglietti», interruttori **contatto** e **compagno/a** (quest'ultimo solo se il compagno/a c'è) e numero **ospiti senza nome** (0-50); ogni cambio si salva subito; se non resta nessun posto il biglietto si toglie (con conferma); «+ Biglietto BBS/WES» → sceglie tra le date non ancora usate (`MB21Lista.eventiLiberi`), parte con il contatto spuntato · riquadro **CEP** solo se Partner: «Abbonato ora» / «Non abbonato ora», un blocco per **periodo** (dal più recente) con «Abbonato dal» · «Uscito il» · **Elimina** · **Salva** (si salva **solo col bottone**: salvando a ogni cambio l'anno non si poteva scrivere), «+ Nuovo periodo» apre un blocco vuoto (Annulla / Salva). `MB21Lista.controllaPeriodoCep`: data obbligatoria, anno 1990-2100, uscita non prima dell'abbonamento, un solo periodo aperto, niente sovrapposizioni. Biglietti e CEP **li modifica solo l'Admin**, gli altri li vedono spenti. `lista.js`: `postiBiglietto(b)` = contatto + compagno/a + ospiti
