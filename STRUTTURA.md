...
- ancora da fare: conteggio a cascata sull'albero e pillole della Mappa, BBS/WES/CEP fuori dal check giornaliero
...
- **segni vitali a cascata** (cantiere 18 lavoro 3 parte 2, 16/09): `caricaSchedeMappa` legge anche biglietti da oggi in poi e periodi CEP (con `contatti(user_id)`), coppie collegate, `utenti.partner_id`, prossimo BBS e prossimo Wes. `MB21Mappa.segniGruppo(d)` → `{ proprio, gruppo }` per partner: ogni biglietto/periodo va al partner della **scheda** (codice Amway o nome, `schedaDelPartner`) o del **compagno/a collegato**; altrimenti al partner **proprietario della lista** (`utenti.partner_id`). Conta i **posti del prossimo BBS**, i **posti del prossimo Wes** (`prossimaData`) e gli **abbonati CEP di oggi** (un periodo = 1); `gruppo` = lui + tutti quelli sotto (guardia 30 livelli). Testata «Segni vitali del gruppo: prossimo BBS … · prossimo Wes … · CEP oggi»; sulla riga il **totale del gruppo dentro la targhetta** («BBS 7», se sopra zero): il **colore** dice se il partner stesso ha il segno acceso, il **numero** conta tutto il gruppo, quindi una targhetta grigia può avere un numero (richiesta di Ignazio 16/09: niente seconda riga di scritte sul telefono). Un partner vede solo i biglietti dei contatti che può leggere
...
- **Lista Nomi** (Fase 2, copia della tab di Glide → `docs/MB21_v3_Lista_come_e.md`):
  ...
  - **Targhette BBS · WES · CEP** accanto al nome nella testata (cantiere 18, 16/09): grigie, colorate (blu · rossa · verde) se accese. `MB21Lista.targheSegni(biglietti, periodiCep, oggi)`: BBS/WES accese se c'è un biglietto con posti per un evento **non ancora passato** (si spengono da sole dopo l'evento); CEP accesa se oggi è dentro un periodo. Con il compagno/a collegato valgono i segni della coppia. Lettura unica per scheda (`segniDellaScheda`, condivisa con la sezione)
