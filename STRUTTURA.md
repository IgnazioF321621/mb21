...
- **Admin** (richiesta di Ignazio 16/09, pagina per i lavori dell'Admin): riquadro **📄 Carica file Amway** (vedi Import CSV Amway) · **👥 Utenti dell'app** (cantiere 19 lavoro 3): tutti gli utenti (Admin, poi chi può entrare, poi per nome) con email, codice Amway, «✅ già entrato» (`auth_id`) / «mai entrato», interruttori **Può entrare** (`accesso_attivo`; senza email non si accende) e **Nel Partner Select** (`nel_partner_select`, il menu si aggiorna subito); sulla riga dell'Admin spenti · **🎟 Wes** e **🎟 BBS** spostati qui dal Report (elenco con Elimina, mese e anno + «+ Wes» / «+ BBS», conta l'ultimo caricato; un evento con biglietti non si elimina)
