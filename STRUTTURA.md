...
**utenti** (cantiere 19) — `abbonamento_con` (uuid → `utenti.id`, migrazione `20260916183500_abbonamento_in_comune.sql`, applicata): **abbonamento in comune** della coppia (scelta B di Ignazio 16/09: liste separate, un pagamento). Punta a chi paga; vuoto = paga per sé; non può puntare a sé stesso. `abbonamento_scadenza` la scrive l'Admin (pagamenti Revolut controllati a mano)
