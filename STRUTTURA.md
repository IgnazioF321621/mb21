...
- **Foglio unico «Modifica azione»** (`foglioAzione`, decisioni di Ignazio 15/09: un'azione per ogni passo del percorso, il foglio serve a **correggere**): titolo = tipo, nome del contatto, **Sottotipo** (chip) · **Esito** (chip; per Appuntamento cambiano col sottotipo) · **Giorno e ora** (la fine si sposta con la stessa durata) · **Ospite** (PM, Follow Up o se già presente) · **Note**; «👤 Apri contatto» (dove serve) o Annulla · **Salva** → avviso «Azione modificata [· coda aggiornata]» con **Annulla**. Tocco fuori o × chiude. Si apre da: **Report** (tocco su un nome) · **Griglia PM** (tocco su una casella) · **scheda contatto → Azioni → Modifica** (prima solo data e nota) · **Agenda** («✏️ Modifica» accanto a Sposta)
