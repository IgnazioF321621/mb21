...
- ~~Rilievo della tab Mappa~~ — fatto il 16/09 (ripresa automatica, 18 foto, 8 scelte): `docs/MB21_v3_Mappa_come_e.md`. La Mappa **non è geografica**: è l'**albero del gruppo (LOS)** con i volumi del mese. Pagina: Partner Select · riquadro «Mese» (VPP/VPG/Bonus + «VP mancanti al 6%») · grafico «Andamento PY» · «Storico mensile» (13 mesi) · «Prime Linee» (stato 🟢 attivo / ⚪ inattivo / 🔴 warning + livello). Toccando un partner si apre la sua scheda uguale e si **scende di livello** (visto fino al Liv. 4); solo da guardare, niente ricerca né filtri. Dati dell'export: `LOS.csv` (partner+mese) e `Partners.csv` (albero con `Sponsor_ID`). Da verificare: soglie attivo/warning, da dove arrivano VPP/VPG/Bonus, «VP mancanti al 6%», aggiornamento dell'albero (§4 del documento). **Prossimo: domande a Ignazio, una alla volta**
