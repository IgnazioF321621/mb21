...
- **Più in alto = più importante**, sia i lavori dentro un titolo sia i titoli tra loro: il primo lavoro non fatto del primo titolo è il prossimo da fare (da lì si parte quando chiede «cosa facciamo adesso?»). Nell'app (dal 24/09) le fatte si vedono in fondo al loro titolo e i titoli tutti fatti in fondo alla lista (`MB21Agenda.fatteInFondo`): l'importanza conta solo tra le righe da fare. Il posto di un lavoro nuovo lo sceglie Claude; l'ordine dei titoli lo decide Ignazio (trascina il titolo nell'app, o lo chiede). Un titolo si sposta **sempre con tutte le sue righe** (fino al titolo dopo), in un comando solo (`update … set ordine = case … end`), poi si ricontrolla che i numeri vadano da 1 a N senza buchi né doppi.
