...
- **Più in alto = più importante**, sia i lavori dentro un cantiere sia i cantieri tra loro: il primo lavoro non fatto del primo cantiere è il prossimo da fare (da lì si parte quando chiede «cosa facciamo adesso?»). Nell'app (dal 24/09) le fatte si vedono in fondo al loro cantiere e i cantieri tutti fatti in fondo alla lista (`MB21Agenda.fatteInFondo`): l'importanza conta solo tra le righe da fare. Il posto di un lavoro nuovo lo sceglie Claude; l'ordine dei cantieri lo decide Ignazio (trascina il titolo nell'app, o lo chiede). Un cantiere si sposta **sempre con tutte le sue righe** (dal titolo fino al titolo dopo), in un comando solo (`update … set ordine = case … end`), poi si ricontrolla che i numeri vadano da 1 a N senza buchi né doppi.
- **Come:** leggere le righe (`select id, ordine, tipo, livello, fatto_il, testo from cose_da_fare where progetto_id = '56a23b23-daff-4ae4-9066-dba97d55b099' order by ordine, creato_il`), scegliere il posto `P` (il numero d'ordine della riga da fare davanti a cui va; in fondo al cantiere: il numero del titolo dopo), poi un comando solo, che sposta giù di uno le righe da `P` in poi e inserisce (provato il 24/09 in una prova annullata). Si lancia con `supabase db query --linked -f <file>`, con il file scritto da `<<'EOF'`: tra virgolette doppie la shell si mangia i `$t$`.
  ```sql
