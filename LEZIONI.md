...
## Una traccia si consiglia solo se è nel BSM, e con il suo nome del catalogo
*24 settembre 2026, cantiere 42 (il coach).* Nelle note Evernote di Ignazio la riga **«BSM/A:»** dice dove la traccia si trova nel materiale di Network 21. Se è **vuota**, la traccia è un CEP passato che le persone non trovano più: il suo contenuto si usa per scrivere, ma **non si consiglia**. Se è **compilata** e il titolo scritto lì **è quello della nota**, è una **traccia singola**; se è **diverso**, la traccia sta **dentro un pack** (quello nominato nella riga) e si consiglia come «la traccia X nel pack Y». Esempio: la frase «non è l'intenzione, ma la direzione che determina la destinazione» è di Bini in «Il principio del percorso», **nel pack «Dare Seguito 1»**, non nel pack dove l'avevo trovata io. Lo stesso per i libri: le idee si prendono da tutti i PAT, ma si propongono solo i **libri a catalogo** (tabella `materiali`, non quelli aggiunti dagli utenti).
**Regola:** prima di far consigliare una traccia o un libro, controllare la riga BSM (singola o pack) e la tabella `materiali`; scriverla sempre come la cercano le persone: prima **dove** sta nel BSM (sezione o pack), poi l'**oratore**, poi il **titolo** («Nel BSM, pack «Dare Seguito 1»: Massimo Bini – «Il principio del percorso»»). E il libro si dice libro: «il libro di Napoleon Hill «Pensa e arricchisci te stesso»».

## Una domanda che l'app fa già non si rifà con parole diverse
*24 settembre 2026, cantiere 42 (il coach).* Nella prima batteria delle telefonate il coach apriva con «Com'è andata?» e tre risposte sue (Bene · Così così · Male). Ma l'app lo chiede già: il foglio «Com'è andata?» dell'Agenda e i bottoni della coda **sono gli esiti**, e sono quelli che vanno nei report. Ignazio: «le risposte vanno agganciate a quelle… sono lo stesso significato: sono i termini che cambiano». Due domande uguali con parole diverse confondono chi usa l'app e spezzano i numeri in due posti.
**Regola:** prima di aggiungere una domanda, cercare con grep se l'app la fa già (il testo della domanda, i bottoni): se sì, **si riusa la risposta che c'è** (qui l'esito) e si parte da lì. È la stella cometa: una cosa fatta in un punto si ritrova già fatta negli altri.

## Due funzioni con lo stesso nome in due file: vince l'ultima caricata
*24 settembre 2026, Dashboard a righe.* La funzione nuova della Dashboard si chiamava `cambiaSezione`, come una di MB Plan (`pagina-agenda.js`): i file dell'app sono `<script>` normali, tutti nello stesso spazio, e quello caricato dopo **sostituisce** l'altro senza nessun avviso. Il tocco sulla riga «Contatti del giorno» finiva così nel codice dell'Agenda, che si rompeva («Cannot read properties of null») su tutti i dispositivi. Né `node --check` né le prove da banco lo vedono: ognuno dei due file, da solo, è giusto.
**Regola:** ogni volta che si aggiunge una funzione o una costante in cima a un file dell'app, prima si cerca il nome con grep in **tutti** gli altri (`grep -n "nomeFunzione" *.js index.html`); un controllo rapido di tutti i doppioni: `grep -ho "^function [A-Za-z0-9_]*" pagina-*.js admin.js | sort | uniq -d` (deve non stampare niente).

## La misura di un'icona e «hidden» si decidono nello stile, non negli attributi
*24 settembre 2026, cantiere 42 (la pagina Training).* `ic('telefonate', 26)` scrive `width="26"` sull'icona, ma lo stile di tutte le icone (`svg.ic { width: 1em }`) vince sugli attributi: nel cerchio colorato l'icona restava piccola. Stessa cosa per `hidden`: su un elemento con `display: grid` (il fondo dei fogli, `.mc-fondo`) lo stile vince e l'elemento si vede lo stesso (era già successo alla barra in basso, cantiere 32).
**Regola:** la misura di un'icona dentro un contenitore si scrive nello stile del contenitore (`.trn-icona > svg.ic { width: 26px; height: 26px }`); un elemento che si nasconde con `hidden` e ha un `display` suo ha bisogno della sua regola `…[hidden] { display: none }`. Si vede solo guardando la pagina: per questo le foto con l'anteprima rapida prima di dire «fatto».

## Da Admin funziona tutto: le schermate vanno provate nei panni di un partner
*24 settembre 2026, biglietti BBS/WES di Isabella.* Il partner rispondeva «sì, ho il biglietto» e il biglietto veniva scritto davvero (la funzione che scrive sa che la scheda col suo codice Amway sta nella lista dell'upline), ma **Check del Giorno e Modulo Core lo rileggevano con una select diretta su `contatti`**: quella scheda il partner non la può leggere, quindi risultava «non comprato». All'Admin non si vedeva, perché lui le liste le vede tutte: il difetto è rimasto in giro una settimana.
**Regola:** quando un dato sta su una scheda che **non è nella lista di chi guarda** (la propria scheda del partner, i suoi biglietti, il suo CEP, le sue tracce), scriverlo e **rileggerlo** devono passare dalla **stessa strada** (`_scheda_segni` e le funzioni `security definer` che la usano). Prima di dire che una schermata funziona, provarla sul database **nei panni di un partner** (`set local role authenticated` + `request.jwt.claims` con il suo **`auth_id`**, non il suo `utenti.id`, dentro una transazione annullata): se vede zero dove dovrebbe vederne tre, il difetto è lì.

## Le proposte vanno subito nella lista MB App, non in una domanda in chat
*24 settembre 2026, sessione Training.* Alla fine di ogni passo Claude chiudeva con le sue proposte in forma di domanda («Lo metto in MB App come prossimo lavoro?», «Li spunto?»): ogni proposta era una risposta in più da dare, e restava sparsa nella chat invece che nella lista da cui si lavora. Ignazio: «Passiamo ad un livello ancora più elevato. Quando hai delle proposte, inseriscile direttamente… e comunque, in ordine, andiamo a fare tutto quello che ci occorre», per tutti i cantieri, comunque si chiamino.
**Regola:** ogni proposta di Claude (un lavoro che serve, un difetto visto per caso, un passo che manca) va **subito** in MB App, sotto il suo titolo e al posto giusto per importanza, **senza chiedere**; a Ignazio si dice solo cosa è stato messo e dove, una riga per proposta. Poi si lavora **nell'ordine della lista**: il primo lavoro da fare del titolo della sessione è il prossimo passo, uno alla volta, e si spunta col suo «ok». Si chiede solo quello che non si può sapere da soli (per esempio una riga arrivata a metà). Come si inserisce: `CLAUDE.md`, «Promemoria MB».

## Nelle domande a scelta la risposta giusta non deve essere la più lunga
*24 settembre 2026, cantiere 45 (le carte del Training).* Scrivendo le 134 carte del Nuovo, la risposta giusta veniva spiegata bene e le sbagliate buttate lì corte: alla fine era **la più lunga nell'84-100% delle scene**. Così si indovina senza sapere, e il test non misura niente: proprio il contrario di quello che vuole Ignazio («la voglia di imparare e la rabbia se ancora non so le cose»). Lo stesso per i vero o falso: i trabocchetti erano quasi tutti «falso», e si impara a rispondere «falso» a tutto quello che suona strano.
**Regola:** le risposte sbagliate si scrivono **plausibili e lunghe quanto la giusta**, con una loro ragione («…e insistere rovinerebbe il rapporto»); la giusta dice l'essenziale e la frase esatta da imparare va nel perché. La giusta non deve essere la più lunga, né la più corta, in più di 4 versioni su 10; tra i trabocchetti vero o falso almeno un terzo sono veri che sembrano falsi. La prova privata delle carte (`prova_carte.js`) lo controlla prima di caricare un mazzo.
